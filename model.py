...
class Custum_class(tf.keras.Model):
    def __init__(self, model,t_model, **kwargs):
        ...
        self.model = model
        self.teature_model = t_model

    def compile(self, optimizer, metrics, unet_loss_fn, tversky_loss_fn,dice_loss_fn,dist_loss_fn, alpha=0.1, temperature=3, **kwargs):
    ...
    def train_step(self, data):
        ...
        cnn_teature = self.teature_model(x,training = False)
        with tf.GradientTape() as tape:
            ...
            kl_div_loss = self.dist_loss(cnn_teature, cnn_output)

            loss = self.alpha * unet_loss + tversky_loss + dice_loss + kl_div_loss
